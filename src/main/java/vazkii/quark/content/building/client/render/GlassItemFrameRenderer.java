...
public class GlassItemFrameRenderer extends EntityRenderer<GlassItemFrameEntity> {
	...
	public GlassItemFrameRenderer(EntityRenderDispatcher renderManagerIn, ItemRenderer itemRendererIn) {
		...
		bannerModel = BannerRenderer.makeFlag();
		this.itemRenderer = itemRendererIn;
		this.defaultRenderer = (ItemFrameRenderer) renderManagerIn.renderers.get(EntityType.ITEM_FRAME);
	}

	@Override
	public void render(GlassItemFrameEntity p_225623_1_, float p_225623_2_, float p_225623_3_, PoseStack p_225623_4_, MultiBufferSource p_225623_5_, int p_225623_6_) {
		...
		if(p_225623_1_.getEntityData().get(GlassItemFrameEntity.IS_SHINY))
	...
	@Override
	public Vec3 getRenderOffset(GlassItemFrameEntity p_225627_1_, float p_225627_2_) {
	...
	@Override
	public ResourceLocation getTextureLocation(GlassItemFrameEntity p_110775_1_) {
		return TextureAtlas.LOCATION_BLOCKS;
	}

	@Override
	protected boolean shouldShowName(GlassItemFrameEntity p_177070_1_) {
	...
	@Override
	protected void renderNameTag(GlassItemFrameEntity p_225629_1_, Component p_225629_2_, PoseStack p_225629_3_, MultiBufferSource p_225629_4_, int p_225629_5_) {
		super.renderNameTag(p_225629_1_, p_225629_1_.getItem().getHoverName(), p_225629_3_, p_225629_4_, p_225629_5_);
	}

	protected void renderItemStack(GlassItemFrameEntity itemFrame, float p_225623_2_, float p_225623_3_, PoseStack matrix, MultiBufferSource buff, int p_225623_6_, ItemStack stack) {
		if (!stack.isEmpty()) {
			matrix.pushPose();
			MapItemSavedData mapdata = MapItem.getOrCreateSavedData(stack, itemFrame.level);
			...
			if (!MinecraftForge.EVENT_BUS.post(new RenderItemInFrameEvent(itemFrame, defaultRenderer, matrix, buff, p_225623_6_))) {
				if (mapdata != null) {
					...
					this.mc.gameRenderer.getMapRenderer().render(matrix, buff, mapdata, true, p_225623_6_);
				} else {
					float s = 1.5F;
					if (stack.getItem() instanceof BannerItem) {
						...
						this.itemRenderer.renderStatic(stack, ItemTransforms.TransformType.FIXED, p_225623_6_, OverlayTexture.NO_OVERLAY, matrix, buff);
					}
				}
			}
